...
function MenuItemWrapper({ children, option }: { children: ReactNode; option: OptionMenuItemType }) {
  if (option.href) {
    return (
      <Box
        component={Link}
...
function OptionsMenu(props: OptionsMenuType) {
  // ** Props
  const {
    icon, options, menuProps, iconProps, leftAlignMenu, iconButtonProps,
  } = props;
  ...
  return (
    <>
      <IconButton aria-haspopup="true" onClick={handleClick} {...iconButtonProps}>
        {icon || <Icon icon="bx:dots-vertical-rounded" {...iconProps} />}
      </IconButton>
      <Menu
        keepMounted
        anchorEl={anchorEl}
