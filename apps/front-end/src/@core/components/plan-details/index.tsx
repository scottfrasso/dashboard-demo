...
function PlanDetails(props: PricingPlanProps) {
  // ** Props
  const { plan, data } = props;

  const renderFeatures = () => data?.planBenefits.map((item: string, index: number) => (
        ...
        <Box sx={{ my: 7, position: 'relative' }}>
          ...
          {plan !== 'monthly' && data?.monthlyPrice !== 0 ? (
            <Typography
              variant="caption"
              sx={{
                top: 50, left: '50%', position: 'absolute', transform: 'translateX(-50%)',
              }}
            >
