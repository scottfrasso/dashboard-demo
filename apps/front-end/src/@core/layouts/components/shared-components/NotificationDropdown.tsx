// ** React Imports
import {
  useState, SyntheticEvent, Fragment, ReactNode,
} from 'react';
...
function ScrollWrapper({ children, hidden }: { children: ReactNode; hidden: boolean }) {
  if (hidden) {
...
function NotificationDropdown(props: Props) {
  ...
  function RenderAvatar({ notification }: { notification: NotificationsType }) {
    const {
      avatarAlt, avatarImg, avatarIcon, avatarText, avatarColor,
    } = notification;
